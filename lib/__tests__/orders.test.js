...
it('calls the protected order RPCs with stable argument names', async () => {
  supabase.rpc
    ...
    .mockResolvedValueOnce({ data: { id: 'order-1' }, error: null })
    .mockResolvedValueOnce({ data: { id: 'order-1', status: 'confirmed' }, error: null })
  ...
  await expect(updateOrderStatus('DRJIVA-CODE', 'order-1', 'confirmed')).resolves.toEqual({
    id: 'order-1',
    status: 'confirmed',
  });
  ...
  expect(supabase.rpc.mock.calls).toEqual([
    ...
    [
      ...
      { p_access_code: 'DRJIVA-CODE', p_order_id: 'order-1', p_status: 'confirmed' },
    ],
