# DRJIVA Order Dispatch

An installable, single-purpose PWA for receiving DRJIVA cash-on-delivery
medicine orders, sharing them to the owner's WhatsApp riders group, assigning
a rider, and recording delivery progress. Supabase stores the durable queue;
Realtime refreshes open devices; standards-based Web Push alerts installed
...
Unlock the console with the separately delivered high-entropy dispatcher access
code. The plaintext code and VAPID private key are not present in this repo.

## First device setup

1. Open the HTTPS deployment.
2. Unlock with the dispatcher access code.
3. Open Medicine pickup settings and save the real Asian Hospitals address and
...
The owner has explicitly chosen full customer, phone, medicine, COD total,
